...
export const ExerciseForm = (props) => {
  ...
  const { measurementTypes, getMeasurementTypes } = useContext(
    MeasurementTypeContext
  );
  ...
  const FlyingDiv = ({ title = "Let's go!", isHidden }) => {
    ...
    const [divHiddenClass, setDivHiddenClass] = useState(
    ...
    useEffect(() => {
      ...
      if (isHidden === false) {
        ...
        hiddenClassTimer = setTimeout(() => {
          setDivHiddenClass(visibilityClasses.hidden);
        }, 2800);
      } else {
  ...
  const handleETChange = (e) => {
    if (e.target.value !== "0") {
      const exTypeSelected = exerciseTypes.find(
        (et) => parseInt(e.target.value) === et.id
      ).type;
    ...
    <div className="cont--form-ex">
      <section className="form">
        <section className={`steps ${stepOne ? "hidden" : "visible"}`}>
          <h1 className="h1 header__form header__form--ex">{player.name}</h1>
          ...
          <div className="instructions">
            <div
              className={`instructions-step-one ${
                alertOne ? "visible" : "hidden"
              }`}
            >
          ...
          <div className="lets-go-cont">
            {isReady ? <FlyingDiv isHidden={isHidden} {...props} /> : <></>}
          </div>
          <div className="jiggle-container">
            ...
            >
              <img
                className={`next ${isReady ? "jiggle" : ""}`}
                src="https://res.cloudinary.com/heymonicakay/image/upload/v1601408603/wideRetriever/FB962FED-6991-4FCE-8D65-1A3A33211BA9_rnqjrl.png"
                alt=""
              />
            </span>
          </div>
        ...
        <section className={`steps ${stepThree ? "hidden" : "visible"}`}>
          ...
          <div className="note-section">
            <label className="note-label">
              Add a note about today's {exTypeSelectedLower}...
            </label>
            <textarea
              ref={note}
              name="note"
            ...
            <div
              className="discard"
              onClick={() => props.history.push(`/players/${playerId}`)}
            >
